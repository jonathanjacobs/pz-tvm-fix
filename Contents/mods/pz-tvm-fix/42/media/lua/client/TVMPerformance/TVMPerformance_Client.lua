...
local function install()
    ...
    local state = { lastSlice = nil, lastVisualSnapshotByMachine = {} }
    ...
    client.requestVisualRegistrySlice = function(x, y, z, radius, maxRows, hints)
        ...
        if not (px and py) then return originalSlice(x, y, z, radius, maxRows, hints) end
        ...
        if not due and not moved then return false end
    ...
    client.requestSnapshot = function(machineId, uiType, hints)
        ...
        local now = nowMs()
        if now > 0 and not snapshotDue(machineId, now) then return false end
        local sent = originalSnapshot(machineId, uiType, hints)
        if sent == true and now > 0 then markSnapshot(machineId, now) end
        return sent
    end
